---
title: Cortadel × n8n
description: A Cortadel Memory sub-node for the n8n AI Agent's memory port, plus a six-operation action node that doubles as an agent tool.
---
...
- **Nothing is exercised against a live Cortadel server in CI.** The test suite is fully offline:
  ...
  converters. Wire-level behaviour rests on the [SDKs](/sdk-typescript/) and their own
  conformance suites.
