---
title: Cortadel × Microsoft Agent Framework
description: Long-term memory for Microsoft Agent Framework agents in .NET — an AIContextProvider that recalls before the model call and persists after the turn, plus search_memory / add_memories tools.
---
...
- **Nothing runs against a live Cortadel server in CI.** The suite stubs `ICortadelMemory` — the
  ...
  go through — so wire-level behaviour rests on the [.NET SDK](/sdk-dotnet/) and its own
  conformance suite.
...
- **A running Cortadel server** — the hosted service at `https://app.cortadel.ai`, or your own via
  `docker compose up` → `http://localhost:3001` (see [Self-hosting](/self-hosting/)). A
  self-hosted server defaults to auth disabled, in which case `ApiKey` may be omitted; otherwise
  mint a key (see [Authentication](/authentication/)).
