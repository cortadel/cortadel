---
title: Cortadel × Google ADK
description: Long-term memory for Google ADK agents — a BaseMemoryService implementation, an auto-persist BasePlugin, and model-callable search_memory / add_memories tools.
---
...
That pulls in `google-adk>=2.0.0,<3.0.0` and the [`cortadel` Python SDK](/sdk-python/)
(`>=1.0.0,<2.0.0`).
...
| `api_key` | `str \| None` | `$CORTADEL_API_KEY` | Bearer token (see [Authentication](/authentication/)). Omit when the server runs with auth disabled. |
...
- **Offline tests only.** The suite drives real ADK objects — real `Session`, `Event`,
  ...
  wire-level behaviour rests on the [Python SDK](/sdk-python/) and its own conformance suite.
...
- A running Cortadel server — the hosted service at `https://app.cortadel.ai`, or your own via
  `docker compose up` → `http://localhost:3001` (see [Self-hosting](/self-hosting/))
...
- **All integrations** — [Integrations](/integrations/)
