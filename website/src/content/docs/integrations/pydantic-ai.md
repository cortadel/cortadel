...
user_id, ...)` does in the [Python SDK](/sdk-python/) itself. **Every other option is
...
| `api_key` | `str \| None` | `$CORTADEL_API_KEY` | Bearer token (see [Authentication](/authentication/)). Omit when the server has auth disabled. |
...
`FunctionModel`. Wire-level behaviour rests on the [Python SDK](/sdk-python/) and its own
...
- **cortadel** ≥ 1.0.0, < 2.0.0 — the official [Python SDK](/sdk-python/), installed for you
...
- All twelve packages: [Integrations](/integrations/)
