---
title: Cortadel × Claude Agent SDK
description: Long-term memory for the Claude Agent SDK — in-process MCP memory tools plus UserPromptSubmit / Stop hooks.
---
...
`appName`) — the same variables the [`cortadel-memory` plugin](/plugin/) reads, and it throws
...
:::note
Also in this repo: [`cortadel-memory`](/plugin/), a Claude Code **plugin** that does the same
...
- **Two hook events, and no more.** The SDK's `HookEvent` union is much wider — it includes
  ...
  start remains the [`cortadel-memory` plugin](/plugin/)'s job.
...
- [All integrations](/integrations/) · [MCP integration](/mcp/) ·
