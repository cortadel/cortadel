---
title: Integrations
description: First-party Cortadel packages for twelve agent frameworks — memory tools plus automatic recall and persistence.
---
...
the [SDKs](/sdk-python/) and their own conformance suites.
:::
...
| `api_key` / `apiKey` | The bearer token (see [Authentication](/authentication/)). Omit it when the server runs with auth disabled. |
...
Code, use the [`cortadel-memory` plugin](/plugin/) instead; this package is for agents you build in
Python.
