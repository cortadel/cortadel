...
| **Codex** | Skills only. Codex's plugin format has no user-config templating (no `${user_config.*}` substitution), so it cannot express a configurable base URL or carry your API key — the MCP server and hooks are Claude-Code-only. Wire Codex to the MCP endpoint manually per [MCP integration](mcp.md), using `codex` as the `{clientName}` segment: `<base_url>/mcp/codex`. |
