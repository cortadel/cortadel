...
([`@cortadel/sdk`](../sdk-typescript.md)) comes along as a regular dependency.
...
- **The test suite is fully offline** — a fake Cortadel client and a fake `Model`, with several
  ...
  [`@cortadel/sdk`](../sdk-typescript.md) and its own conformance suite.
...
- **A running Cortadel server** — hosted at `https://app.cortadel.ai`, or self-hosted with
  ...
  [Authentication](../authentication.md) for the API key.
...
- [All integrations](../integrations.md) — the other eleven, and the shared option vocabulary.
