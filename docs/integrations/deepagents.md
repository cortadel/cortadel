...
- **The test suite is fully offline.** It drives a real `createDeepAgent` with a fake tool-calling
  ...
  rests on [`@cortadel/sdk`](../sdk-typescript.md) and its own conformance suite rather than on this
  package's tests.
...
- **A running Cortadel server** — the hosted service at `https://app.cortadel.ai`, or your own
  (`docker compose up` → `http://localhost:3001`, see [Self-hosting](../self-hosting.md)). Pass
  `apiKey` unless the server runs with auth disabled — see [Authentication](../authentication.md).
